...
cd ~
sudo git clone https://github.com/vtnplus/nodejs-pool-sumo.git nodejs-pool  # Change this depending on how the deployment goes.
cd /usr/src/gtest
sudo cmake .
sudo make
...
cd /usr/local/src
sudo git clone https://github.com/sumoprojects/sumokoin.git
cd sumokoin
sudo git checkout
sudo curl https://raw.githubusercontent.com/vtnplus/nodejs-pool-sumo/master/deployment/sumokoin_daemon.patch | sudo git apply -v
sudo cmake .
sudo make -j$(nproc)
sudo cp ~/nodejs-pool/deployment/sumokoin.service /lib/systemd/system/
sudo useradd -m sumokoindaemon -d /home/sumokoindaemon
sudo systemctl daemon-reload
sudo systemctl enable sumokoin
sudo systemctl start sumokoin
...
cd ~
sudo git clone https://github.com/vtnplus/poolui-sumo.git poolui
cd poolui
